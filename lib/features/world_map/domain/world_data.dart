...
import '../../cities/domain/city_appearance.dart';
...
enum MovementTerrain {
  ...
  const MovementTerrain(this.label);
  ...
  double get speedFactor => switch (this) {
    plain => GameConfig.grassSpeedFactor,
    water => GameConfig.waterSpeedFactor,
    mountain => GameConfig.mountainSpeedFactor,
    structure => 1,
  };
}
...
class TileCoord {
  ...
  /// 该格子在原生地图中的中心点。
  GamePoint get center => GamePoint(x * 16 + 8, y * 16 + 8);
...
class CityDefinition {
  ...
  /// 建筑在原生地图上的占用范围。
  GameRect get bounds =>
      GameRect.fromLTWH(x * 16, y * 16, width * 16, height * 16);
...
class WorldDefinition {
  ...
  /// 原生地图的像素尺寸。
  GameSize get pixelSize => GameSize(width * 16, height * 16);
  ...
  /// 返回某个位置上的城池。
  CityDefinition? cityAt(GamePoint point) {
    for (final city in cities) {
