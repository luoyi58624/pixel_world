...
class WorldCamera {
  ...
  /// 当前可见区域的逻辑尺寸。
  GameSize viewport = GameSize.zero;

  /// 世界中的镜头中心。
  GamePoint center = GamePoint.zero;

  /// 原生地图像素到逻辑像素的放大倍数。
  double scale = 3;

  GamePoint _inertiaVelocity = GamePoint.zero;
  ...
  void endDrag(GamePoint screenVelocity, {bool allowInertia = true}) {
    if (!_dragging) return;
    _dragging = false;
    _stopInertia();
    if (!allowInertia ||
  ...
  void advanceInertia(double dt) {
    ...
    if (_inertiaRemaining < 1e-9 || _inertiaVelocity == GamePoint.zero) {
      _stopInertia();
    }
  }
  ...
  void _stopInertia() {
    _inertiaVelocity = GamePoint.zero;
    _inertiaRemaining = 0;
  }
  ...
  /// 当前镜头在世界中覆盖的矩形。
  GameRect get visibleWorld => GameRect.fromCenter(
    center: center,
  ...
  /// 将屏幕局部坐标转为世界坐标。
  GamePoint toWorld(GamePoint local) =>
      center + (local - viewport.center(GamePoint.zero)) / scale;

  /// 将世界坐标转为屏幕局部坐标。
  GamePoint toScreen(GamePoint world) =>
      (world - center) * scale + viewport.center(GamePoint.zero);

  /// 更新视口并保持镜头位于合法范围。
  void resize(GameSize value) {
    cancelMotion();
    viewport = value;
  ...
  /// 以指定屏幕位置为锚点设置缩放倍率。
  void zoomTo(double value, GamePoint local) {
    cancelMotion();
  ...
  /// 应用一次同时包含平移与缩放的手势。
  void transform(GamePoint worldAnchor, double value, GamePoint local) {
    _stopInertia();
    scale = value.clamp(minScale, 6.0);
    center = worldAnchor - (local - viewport.center(GamePoint.zero)) / scale;
    constrain();
  }

  /// 平移屏幕距离，适用于方向键或拖动。
  void pan(GamePoint screenDelta) {
    cancelMotion();
  ...
  void overview() {
    cancelMotion();
    scale = minScale;
    center = worldSize.center(GamePoint.zero);
    constrain();
  }
}
