...
import '../../heroes/presentation/hero_retreat_button.dart';
...
class _BattleSceneState extends State<BattleScene> {
  ...
  bool _fitted = false;
  String? _selectedId;
  GamePoint _anchor = GamePoint.zero;
  double _scale = 1;
              ...
              Expanded(
                child: LayoutBuilder(
                  builder: (context, viewport) {
                    ...
                    if (camera.viewport != viewport.biggest.toGame) {
                      camera.resize((viewport.biggest).toGame);
                    }
                    if (!_fitted) {
                      camera.overview();
                      _fitted = true;
                              ...
                              child: Column(
                                children: [
                                  _cameraButton(
                                    ...
                                    () => camera.zoomTo(
                                      camera.scale * 1.25,
                                      camera.viewport.center(GamePoint.zero),
                                    ),
                                  ),
                                  if (!tight)
                                    _cameraButton(
