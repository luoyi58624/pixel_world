...
extension _CountryStrategy on CampaignState {
  ...
  bool _tryAiWeapon(WorldBattle battle) {
    final sim = battle.simulation;
    if (!sim.canUseWeapon || sim.elapsed < battle._nextAiWeaponDecision) {
      return false;
    }
    ...
    for (final hero in [battle.defender, battle.attacker]) {
      ...
      final side = hero == battle.attacker
          ? BattleSide.attacker
          : BattleSide.defender;
      final opposing = side == BattleSide.attacker
          ? BattleSide.defender
          : BattleSide.attacker;
      final ownTotal = hero.hp + hero.squad.fold<double>(0, (n, s) => n + s.hp);
      final enemyTotal =
          other.hp + other.squad.fold<double>(0, (n, s) => n + s.hp);
      final danger =
          ownTotal / _expectedDamage(sim.basePower(opposing)) <
          enemyTotal / _expectedDamage(sim.basePower(side)) * 1.2;
      if (!danger &&
          !(battle is CityBattle &&
              battle.initialCityLevel >= 3 &&
              enemyTotal > _expectedDamage(sim.basePower(side)) * 2)) {
        continue;
      }
      final slots = List.generate(hero._weaponIds.length, (i) => i)
          .where(
            (i) => weaponCatalog.weapons[hero._weaponIds[i]]!.selfDamage == 0,
          )
          .toList();
      slots.sort((a, b) {
        final x = weaponCatalog.weapons[hero._weaponIds[a]]!,
            y = weaponCatalog.weapons[hero._weaponIds[b]]!;
        return (x.damage - enemyTotal).abs().compareTo(
          (y.damage - enemyTotal).abs(),
        );
      });
      if (slots.isNotEmpty &&
          useWeapon(hero, slots.first, countryId: hero.countryId)) {
        return true;
      }
    }
    return false;
