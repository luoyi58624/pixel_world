...
class CityContact {
  ...
  Offset departure(Offset center, Offset toward) {
    var direction = toward - center;
    if (direction.distanceSquared < _epsilon) direction = const Offset(1, 0);
    final end =
  ...
  Offset approach(Offset from, Offset aim) {
  ...
  double? entryFraction(Offset from, Offset to) {
    if (!Rect.fromPoints(from, to).inflate(_epsilon).overlaps(bounds) ||
        contains(from)) {
      return null;
    }
  ...
  List<double> _crossings(Offset from, Offset to) {
    final delta = to - from;
    final result = <double>[];
    for (var i = 0; i < _outline.length; i++) {
