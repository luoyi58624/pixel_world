...
英雄商店从本地图的共享回收池随机抽一位英雄，每次花 5 金币，**每城每月最多抽一次**，池中英雄等概率且不会与在场或待签约英雄重复。各国各自保留一个签约结果，互不阻塞；签约、放弃、关闭面板或城池易主均不重置该城当月次数。普通将领免费签约，高级将领另付 10 金币，也可以放弃；放弃将英雄退回池中，抽取费不退。未登场、阵亡和失城被移除的非主角英雄均可入池，主角永不入池。抽取结果保留到处理，关闭面板不重复抽取；签约费不足可等待月结。签约后英雄满血进驻招募城，初始不带兵，需从储备配兵。招募城易主时待签约结果返回池中。
...
各国的 `CountryConfig.initialGold` 是国家初始资金，`garrisonHeroes` 是**每座城市**在自动出征后至少留下的存活将领数；玩家国家仍由玩家指挥。默认奥尔梅、鲍罗布、本塔每城留 2 位，其他国家留 1 位。这些资金和留守数量属于新游戏配置，初始英雄、城池等级继续读取 ROM 提取地图。AI 优先补兵和招募，选择内政最高的驻军主持升级；高级签约费不足时先存钱。超过留守要求且配足 4 兵的将领可出征，战斗属性最高者优先，同值比较生命上限、当前生命和编号；一级城仍只允许同时派出一位。升级费用、兵价、签约费和月俸均从行动国家的国库支付，不共享玩家资金。
