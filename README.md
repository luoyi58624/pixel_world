...
所有国家从第一游戏月即可招募将领，不限制城防、人数和每月次数。抽取五金币，签约立即支付首月月俸，关闭窗口放弃；国家共享回收池，不重复英雄。AI 允许单将先攻前排、持续轮攻补充战损，不再等待全队打穿整城。武器商店在右下角，每年解锁一行，参战英雄每轮对阵最多自动使用一件。
...
初始英雄都在所属城池内，城外没有预览人物。任意等级的城池均可连续派出多位可用将领，每位独立选择目标并带领自己的士兵。出征者立即移出城池的驻军名单和属性区；正式进入攻城战后在大地图和小地图隐藏，只保留城上交战标记，排队和野战角色仍显示；全部出征后显示空城驻军状态，进驻本国城池后重新出现。归属本城的在外英雄继续支付月俸，不能重复出征或远程补兵、升级。进攻战败只移除出征英雄及其部队，不改变出发城的等级、归属、产出和驻军。攻城期间每胜一轮仅降低下一轮的临时城防加成；整场未攻下时，每个胜轮独立按 80% 概率降低一级实际城防，最低一级。赢满初始城防等级对应的轮数或清空守将即可占领，易主重置一级，未出战的败方英雄全部移除，其他城池英雄不受影响，本城在外英雄按失城清除规则处理。升级不生成新英雄，英雄进驻本国或新占领的城池后恢复满 HP，存活守将赢得攻城战后也恢复满 HP；守城开战前从全国库存领最多四兵，结束后生还者归营。出征扣除携带兵员，回城只归还生还者并舍弃超额。城池易主调整双方全国兵员上限，不移交库存，也不赠兵。
...
普通拼杀使用从原 ROM 提取的加速、反弹、随机伤害、共享兵力、随机减员和退场计算片段，保留独立原版测试夹具。新游戏在此基础上使用平衡配置：每兵 20 生命、2 点整队碰撞强度，城防每级加一点基础攻击，不加士气；士气从英雄 JSON 独立读取，上限 100。击退与撞墙倍率分别为 0.25、0.5。三级城连续迎战的临时加成依次为 +3、+2、+1，实际建筑和城防在整场结束时才结算。
