...
`GameConfig.countries` 可直接修改每国资金和留守数量；`countryAiInitialDelay` / `countryAiInterval` 控制决策节奏，`countryAiMinimumSoldiers` 控制出征兵力，`heroDrawsPerCityPerMonth` 控制招募次数。`countryAiEnabled` 可关闭自动经营。国家已无城时不能远程购买或招募；失城时在外部队依照立即清除或打完当前交战后清除的规则处理。地图只显示实际出征或扎营的部队，驻城英雄从城池面板选择，敌军只可查看。
...
初始英雄都在所属城池内，城外没有预览人物。任意等级的城池均可连续派出多位可用将领，每位独立选择目标并带领自己的士兵。出征者立即移出城池的驻军名单和属性区；全部出征后显示空城驻军状态，进驻本国城池后重新出现。归属本城的在外英雄继续支付月俸，不能重复出征或远程补兵、升级。进攻战败只移除出征英雄及其部队，不改变出发城的等级、归属、产出和驻军。只有守城英雄战败才降低实际被攻打的城池等级；若战败前已经是一级，该城易主且未出战的败方英雄全部移除，其他城池英雄不受影响，本城在外英雄按失城清除规则处理。升级不生成新英雄，胜利英雄进驻新城并保留当前 HP。
...
普通拼杀现已使用从原 ROM 提取的计算片段：逐帧加速、双向反弹、红条输入、随机伤害、共享兵力、随机减员和弹地退场。没有沿用旧版固定冷却与百分比平移。小兵为整队提供 20 兵力和 2 点碰撞强度；英雄战斗 15、4 兵且无修正时，一次普通接触造成 7–13 点整队伤害，而非给每名小兵各扣一遍。守城修正默认每级 +2，红条按战斗属性初始化，上限 63；场地修正只在开场读取。
