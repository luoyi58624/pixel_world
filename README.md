...
常用规则集中在 [`lib/game_config.dart`](lib/game_config.dart)：年月长度、各国初始金币、收成权重、城池收入与升级费、储备容量、征兵价格、抽取与签约费、月俸折算、行军地形速度、小兵属性及冲锋士气。修改后热重启，重新开始本局即可按新配置初始化。原 ROM 英雄属性目录保持不变，主角无月俸是固定身份规则。
