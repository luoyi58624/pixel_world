...
| 点击城池上方刀剑 | 查看后台战斗的实时 HP、轮次和战斗记录，返回地图不停止交战 |
| 点击或拖动小地图 | 快速定位 |
| 空格 | 回到初始据点 |
| F | 查看全图 |
| G | 显示网格 |
...
- `lib/ui/unit_panel.dart`：角色操作、属性与实时观战面板。
...
抵达敌城后按每秒一次的简易顺序公式在后台交战，同一城池同时只处理一支进攻部队，其他部队等待。城池上方显示动态交叉刀剑，点击标记或城池中的观战入口可查看双方实时 HP、轮次、伤害和结束结果。角色面板、选点和观战共用游戏时钟，不暂停行军、交战或经济；交战英雄收到移动、扎营指令时可停止本次进攻。

当前观战先提供实时战况，完整战场表现等待用户后续参考视频。交战公式是新游戏原型，不是 NES 原版战斗算法，细节见提取报告。尚未实现召唤、招募、补兵或存档。切换地图保留本次运行中的城池、国库、英雄和出征状态，未显示的地图暂停；重启应用重新初始化。
