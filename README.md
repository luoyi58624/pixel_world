...
各国的 `CountryConfig.initialGold` 是国家初始资金；电脑每座城市的最低留守人数为该城开局 JSON 等级减一，一级留零位、二级留一位、三级留两位，之后升级不改变基准。多余且配足四兵的将领优先出征，战斗属性最高者优先，同值比较生命上限、当前生命和编号；进攻目标先筛掉补给预算不足者，再加权随机：实际行军越短的邻国越容易被进攻，当前拥有城池越多的国家越容易遭到各国共同施压；玩家扩张也遵守同样规则，城池易主后实时重算。各城共享国库预算：先预留在外部队的行军、排队与交战粮草，以及欠收情况下的月俸缺口，再保留五金币应急，余款才用于补兵、招募和升级。默认预测至少九十秒，长途行军自动延长；招募计入新将领月俸，每派一队立即重算。征兵只补当前驻军需求，资金紧张的在外部队优先回城，旧营地逐队评估能否重新出征。

`campaign_config.json` 的 `countries` 可直接修改每国资金及各城初始等级；`countryAiInitialDelay` / `countryAiInterval` 控制决策节奏，`countryAiMinimumSoldiers` 控制出征兵力，`heroDrawsPerCityPerMonth` 控制招募次数。`countryAiEnabled` 可关闭自动经营。国家已无城时不能远程购买或招募；失城时在外部队依照立即清除或打完当前交战后清除的规则处理。地图只显示实际出征或扎营的部队，驻城英雄从城池面板选择，敌军只可查看。
