...
点击任意城池直接展开单层面板，顶部国名旁显示所属国家金币总数。城防、驻守士兵、招募英雄三个方块横向并排，整块可点；城防只显示当前等级与按选中将领内政折算的实际升级费。征兵每次最多 10 人，人数同时受金币和剩余储备容量限制；三个方块与属性方块同高，左右数值使用相同小字号；驻兵显示人数/容量（如 10/25），满额时右侧显示“已满”，招募人数使用“位”；其余操作右侧只显示金币价格；可操作时价格为金色，不能操作时变灰禁用。招募块不展示“回收池”等额外标签，抽中的英雄在下方直接签约或放弃。敌国方块只读。下方我方英雄可切换选择，显示 HP、战斗、内政、月俸和兵力；经济区仅保留收支和月结报告，移除规则说明、最近记录和敌军名单。底部取消在左、出击在右，确认有效目的地时才自动补兵并离城，取消选点不会扣兵。
