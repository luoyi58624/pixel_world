...
各国的 `CountryConfig.initialGold` 是国家初始资金；电脑每座城市的最低留守人数为该城开局 JSON 等级减一，一级留零位、二级留一位、三级留两位，之后升级不改变基准。多余且配足四兵的将领优先出征，战斗属性最高者优先，同值比较生命上限、当前生命和编号；进攻目标从付得起全程补给的敌城中随机选择。各城共享国库预算：先预留在外部队的行军、排队与交战粮草，以及欠收情况下的月俸缺口，再保留五金币应急，余款才用于补兵、招募和升级。默认预测至少九十秒，长途行军自动延长；招募计入新将领月俸，每派一队立即重算。征兵只补当前驻军需求，资金紧张的在外部队优先回城，旧营地逐队评估能否重新出征。
