...
开局为 1 年 1 月，每 60 秒结算刚结束的月份并进入下一月，12 月后跨年。所有国家各有独立国库，初始资金读取玩法 JSON 的 `countries[].initialGold`，玩家默认 50，其余国家默认 40–80 金币；每个有城池的国家独立抽取收成：50% 正常、25% 欠收、25% 丰收。正常收入为各城月收入之和，丰收再加 `5 × 城池数`，欠收再减 `10 × 城池数`，最后扣该国存活英雄月俸，国库最低为零。月俸读取 `rom_heroes.json` 的 `heroes[].salary`：主角免费且不占名额，从泽拉斯开始，第 1～5 位为 3，第 6～10 位为 2，第 11～15 位为 1，其余为 0；每位均可单独调整。顶栏显示年月和我方金币，城市面板显示所属国家的金币与上月收支；打开面板不会暂停月份。

每个国家只有一份兵员储备，同国所有城池显示相同库存并共同征兵、调兵。上限为“全国城防等级总和 × 4 + 全国存活将领数 × 4”；例如一级城、二级城加三位将领，上限为 24。将领只计一次，出征和同国转城不改变上限。开局将各城 JSON 的 initialReserveSoldiers 相加，统一校验全国容量。征兵仍为一金币一人，每次最多十人；出征或守城开战从全国库存领最多四兵，有多少领多少。回到任意友城或结束守城时，只归还生还兵员，超过全国上限的部分舍弃。战败兵员不归还，取消或重复指令不重复扣兵。占城和升级只增加上限，不赠兵、不接收敌国库存；失城、降级或阵亡按新上限裁掉超额，国家失去全部城池才清空其库存。
