...
已经接触或进入城堡实体范围的部队直接按入城／攻城队列处理，不再要求先走回墙边。AI 调防获准使用的和平临时整备名额会随任务传到实际入城检查；遇敌或满员仍会重新拒绝。行军连续 20 游戏秒无实际位移时重建局部路线，再次无进展则交回国家规划，正常攻城排队、候发、撤退和断粮不触发该恢复。目标被第三国占领后先评估原城与其他敌城，不再直接取消远征返程。验证范围见 [行军与入城恢复记录](docs/army_recovery_2026_09_11.md)。
...
当前经济、兵力、将领月俸、无限次招募、年度城防升级限制与灭国战利品规则统一见 [战役配置](docs/campaign_config.md)。城防第一年可升至三级、第二年四级、第三年起五级；升级不增加收入。每城固定产出10，国家基础收入为丰收30／正常20／欠收0，概率相等。本国核心将领与外聘将领都支付人物月俸；高级3～5、普通1～3，主角当前为0。驻军人数不限，额外驻军费取消，野外粮草照常；解雇返还内政金币。
...
初始金币与城防在 `campaign_config.json` 配置；不再设置固定留守人数，策略参数使用 `GameConfig.nationalAi`。升级、招募和出征都检查现有部队粮草、月俸与月结前后现金低点，不预支未来占城收益。最新规则见 [战役配置](docs/campaign_config.md)，模拟的适用范围和局限见 [本轮验收记录](docs/ai_balance_2026_09_11.md)。
