...
点击任意城池直接展开单层面板，顶部国名旁显示所属国家金币总数。城防、驻守士兵、招募英雄三个方块横向并排，整块可点；城防只显示当前等级与按选中将领内政折算的实际升级费。征兵每次最多 10 人，人数同时受金币和剩余储备容量限制；三个方块与属性方块同高同字号，下方只显示等级或人数，右侧只显示金币价格；可操作时价格为金色，不能操作时变灰禁用。招募块不展示“回收池”等额外标签，抽中的英雄在下方直接签约或放弃。敌国方块只读。下方我方英雄可切换选择，显示 HP、战斗、内政、月俸和兵力；经济区仅保留收支和月结报告，移除规则说明、最近记录和敌军名单。底部取消在左、出击在右，确认有效目的地时才自动补兵并离城，取消选点不会扣兵。
...
城市初始等级由玩法 JSON 配置，初始值沿用原地图，例如奥尔梅分别为二、三、四级，阿尔马均为一级。最高五级，月收入为本城 `baseIncome + (等级 - 1) × 5`；阿尔马一级基础产出 10、奥尔梅一级基础产出 20，其余先设 10；升级需选择一位本城驻守将领；一级升二级基础费用 30 金币，之后每级加 10（30、40、50、60），实付为基础费用减将领内政，最低零。出征、扎营或交战中的将领不能主持升级，成功后将领继续驻守；价格参数可在配置类调整。
...
初始英雄都在所属城池内，城外没有预览人物。任意等级的城池均可连续派出多位可用将领，每位独立选择目标并带领自己的士兵。出征者立即移出城池的驻军名单和属性区；全部出征后显示空城驻军状态，进驻本国城池后重新出现。归属本城的在外英雄继续支付月俸，不能重复出征或远程补兵、升级。进攻战败只移除出征英雄及其部队，不改变出发城的等级、归属、产出和驻军。只有守城英雄战败才降低实际被攻打的城池等级；若战败前已经是一级，该城易主且未出战的败方英雄全部移除，其他城池英雄不受影响，本城在外英雄按失城清除规则处理。升级不生成新英雄，英雄进驻本国或新占领的城池后恢复满 HP；城池易主后储备重置为 10，重复友军进驻不刷新。
