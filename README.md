...
普通拼杀使用从原 ROM 提取的加速、反弹、随机伤害、共享兵力、随机减员和退场计算片段，保留独立原版测试夹具。新游戏在此基础上使用平衡配置：每兵 20 生命、2 点整队碰撞强度，城防一至五级攻击加成为 +1、+3、+5、+8、+12，不加士气；士气从英雄 JSON 独立读取，上限 100。击退与撞墙倍率分别为 0.25、0.5。三级城连续迎战的临时加成依次为 +5、+3、+1，实际建筑和城防在整场结束时才结算。
