...
非玩家国家各自锁定目标并筹备进攻，也会侦测来敌、优先守家。默认从首个逻辑帧开始逐国出兵，首次每帧一个国家，随后每国平均五秒决策一次。满足留守与粮草预算时，多余将领全面出击，优先进攻可胜的弱国；强城先准备武器、兵力和整队粮草，多将对同一目标轮攻，资金不足先积蓄。发现来敌后优先升级、招将、补兵，不再发起新的外攻。具体规则和性能设计见 [国家战略](docs/country_strategy.md)。

各国的 `CountryConfig.initialGold` 是国家初始资金；电脑每座城市的最低留守将领数读取玩法 JSON 的 worlds[].cities[].requiredGarrison，默认两位，与城防等级分开配置。多余将领优先分散进攻不同可胜目标，高级将领先行；目标按国力、当地守军、路程与仇恨加权，默认优先弱国。每支敌军实际抵达城下，被侵略国对来犯国仇恨加20，同一支出征军对同国只计一次；野战双方各加20，上限100，满值提供三倍国别目标权重。各城共享国库预算：先预留在外部队的行军、排队与交战粮草，以及欠收情况下的月俸缺口，再保留五金币应急，余款才用于补兵、招募和升级。默认预测至少九十秒，长途行军自动延长；招募计入新将领月俸，轮攻的全部成员和装备统一核算后才出发。征兵只补当前驻军需求，资金紧张的在外部队优先回城，旧营地恢复资金后优先回城整备。

`campaign_config.json` 的 `countries` 修改国家资金，`worlds[].cities[]` 修改初始城防与 `requiredGarrison` 留守将领数；`countryAiInitialDelay` / `countryAiInterval` 控制决策节奏，`heroDrawsPerCityPerMonth` 控制招募次数。`countryAiEnabled` 可关闭自动经营。国家已无城时不能远程购买或招募；失城时在外部队依照立即清除或打完当前交战后清除的规则处理。地图只显示实际出征或扎营的部队，驻城英雄从城池面板选择，敌军只可查看。
