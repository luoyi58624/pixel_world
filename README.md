...
各国的 `CountryConfig.initialGold` 是国家初始资金；电脑每座城市的最低留守人数为该城开局 JSON 等级减一，一级留零位、二级留一位、三级留两位，之后升级不改变基准。多余且配足四兵的将领优先出征，战斗属性最高者优先，同值比较生命上限、当前生命和编号；目标在所有其他国家城池中随机选择。电脑补兵、招募和内政升级均付真实费用，并保留至少一金币用于出征粮草。
