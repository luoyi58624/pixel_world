...
已接入十五种原版一次性武器。驻城英雄详情可展开军械库，购买或取用国家库存，每将最多三件，战斗中点击使用；直接伤害先扣小兵，溢出再伤将。十二种商店武器按城池数解锁，三种事件专属武器保留初始库存配置。原值、来源与玩法可调字段见 [武器提取说明](docs/nes_weapons.md) 和 [rom_weapons.json](assets/data/rom_weapons.json)。

非玩家国家各自锁定目标并筹备进攻，也会侦测来敌、优先守家。默认开局 8 秒后开始按国家错峰，每国平均五秒决策一次；一次逻辑帧最多处理一个国家。弱城优先派足够的高级将领；强城先准备武器、兵力和整队粮草，多将对同一目标轮攻，资金不足先积蓄。发现来敌后优先升级、招将、补兵，不再发起新的外攻。具体规则和性能设计见 [国家战略](docs/country_strategy.md)。

各国的 `CountryConfig.initialGold` 是国家初始资金；电脑每座城市的最低留守人数为该城开局 JSON 等级减一，一级留零位、二级留一位、三级留两位，之后升级不改变基准。从多余将领中选择足够完成本次目标的队伍，高级将领先行，再比较战斗、生命和编号；进攻目标先筛掉补给预算不足者，再加权随机：实际行军越短的邻国越容易被进攻，当前拥有城池越多的国家越容易遭到各国共同施压；玩家扩张也遵守同样规则，城池易主后实时重算。各城共享国库预算：先预留在外部队的行军、排队与交战粮草，以及欠收情况下的月俸缺口，再保留五金币应急，余款才用于补兵、招募和升级。默认预测至少九十秒，长途行军自动延长；招募计入新将领月俸，轮攻的全部成员和装备统一核算后才出发。征兵只补当前驻军需求，资金紧张的在外部队优先回城，旧营地恢复资金后优先回城整备。

`campaign_config.json` 的 `countries` 可直接修改每国资金及各城初始等级；`countryAiInitialDelay` / `countryAiInterval` 控制决策节奏，`heroDrawsPerCityPerMonth` 控制招募次数。`countryAiEnabled` 可关闭自动经营。国家已无城时不能远程购买或招募；失城时在外部队依照立即清除或打完当前交战后清除的规则处理。地图只显示实际出征或扎营的部队，驻城英雄从城池面板选择，敌军只可查看。
...
初始英雄都在所属城池内，城外没有预览人物。任意等级的城池均可连续派出多位可用将领，每位独立选择目标并带领自己的士兵。出征者立即移出城池的驻军名单和属性区；全部出征后显示空城驻军状态，进驻本国城池后重新出现。归属本城的在外英雄继续支付月俸，不能重复出征或远程补兵、升级。进攻战败只移除出征英雄及其部队，不改变出发城的等级、归属、产出和驻军。攻城期间每胜一轮仅降低下一轮的临时城防加成；整场未攻下时，每个胜轮独立按 50% 概率降低一级实际城防，最低一级。赢满初始城防等级对应的轮数或清空守将即可占领，易主重置一级，未出战的败方英雄全部移除，其他城池英雄不受影响，本城在外英雄按失城清除规则处理。升级不生成新英雄，英雄进驻本国或新占领的城池后恢复满 HP，存活守将赢得攻城战后也恢复满 HP；守城开战前从全国库存领最多四兵，结束后生还者归营。出征扣除携带兵员，回城只归还生还者并舍弃超额。城池易主调整双方全国兵员上限，不移交库存，也不赠兵。
...
普通拼杀现已使用从原 ROM 提取的计算片段：逐帧加速、双向反弹、红条输入、随机伤害、共享兵力、随机减员和弹地退场。没有沿用旧版固定冷却与百分比平移。小兵为整队提供 20 兵力和 2 点碰撞强度；英雄战斗 15、4 兵且无修正时，一次普通接触造成 7–13 点整队伤害，而非给每名小兵各扣一遍。守城修正按当前平衡为每级 +4（一级 +4、五级 +20），红条按战斗属性初始化，上限 63；场地修正只在开场读取。三级城连续迎战的临时加成依次为 +12、+8、+4，实际建筑和城防在整场结束时才结算。
...
出征的进攻将领与野战双方可撤退，守城将领暂不开放。观战和角色面板显示撤退入口：60% 失败并阵亡，成功保留伤势与残兵，播放转身离场后按来路返回出发城。交战时不能再用移动、扎营免费脱战；返回地图仍只退出观战。AI 高级将领观察实际碰撞，在明显劣势时使用相同概率撤退，返程粮草按所有路段预留。返程到家才恢复生命并归还生还兵员，途中断粮暂停、有钱自动续行，第三国仍可拦截。规则与可调参数见 [战斗撤退](docs/battle_retreat.md)。
