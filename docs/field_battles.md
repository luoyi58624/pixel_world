...
地形取相遇点，不取出发城或目的地。山地将领攻击保留 50%，河流 70%，草地/树林及普通地面 90%；当前桥梁等结构地形沿用草地场景与规则。英雄攻击乘地形倍率后向下取整，每名存活小兵提供 1 点碰撞强度、20 点兵力，阵亡立即扣除强度。野战没有城池攻击和士气加成，红条上限为 63；双方使用同一套普通拼杀规则，详见 [内核接入说明](nes_battle_rebuild.md)。
