...
敌对部队行军或扎营时，中心距离到达 16 个地图原生像素便触发一对一野战。同国部队不交战，已经交战或正在城下待战的部队不重复进入野战。检测使用每个固定时间步的相对移动线段，双方同时停在相遇处；第三支部队继续自己的行动。

地形取相遇点，不取出发城或目的地。山地将领攻击保留 50%，河流 70%，草地/树林及普通地面 90%；当前桥梁等结构地形沿用草地场景与规则。只对英雄攻击应用倍率，允许小数，小兵仍为 1 攻击、20 HP。野战没有城池攻击和士气加成，双方士气最高 100，伤害同步结算，边界压制对双方采用相同规则。

野战期间不能改道或扎营。胜方保留血量及兵损，继续原来的目的地；原来驻扎者获胜后继续驻扎。败方英雄按既定回收规则处理，出发城不降级、不易主。主角阵亡仍会结束游戏。野战在后台运行，可以点击地图刀剑标记或角色面板的“进入观战”，返回地图不停止战斗。
