...
3. 全国联合比较防御、回援、调防、在途截击、撤离、整备和扩张，共用国库、兵员及入城预留。
...
明确威胁下优先修复 `H > B`，覆盖留守偏好和招募容量。比较高内政主持升级、最少解雇筹款、合法调防、在外核心回援和在途截击。守将沿实际名单反向出场。主环境接敌和换守将前另有有限保护，只能合法升级或解雇，不能改血量、强移锁定英雄或增加金币。
...
目标结合城防、实际守军、产出、地形路费、占城规模和有限仇恨比较。明显更好的弱目标优先，接近同分才用独立种子打破平局。强城先准备自动领取的兵员和排队粮草；无法供养整队就保留筹备状态，不零散送将。

路线沿真实地形边界积分，有限绕点会检查中途敌城和可见野外敌军。来敌只按位置、最近运动和公开交战状态推断，不读取隐藏命令、未来随机结果。

预算逐英雄保存粮草零头，覆盖去程、排队、交战余量、返程及月俸；检查每月到账前后的现金低点和欠收情况，长途自动扩展范围。未占领城池的未来收入不提前花。采购、最高可能签约费、新增月俸、解雇返款及容量裁兵均入同一本账。

进攻不再存在配装条件，采购与出征只核算兵员和国库；战力评估使用将领、士兵、城防与士气的公开属性。
