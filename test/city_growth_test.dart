...
void main() {
  ...
  test('升级扩建后点击和出城使用新范围，友军进驻不重置等级', () {
    ...
    expect(home.bounds.contains(addedWing), isFalse);
    expect(c.cityAt(addedWing), home);
    final hero = c.heroes.firstWhere((hero) => hero.sourceId == 0);
    final march = c.dispatchTo(
      hero,
      expanded.centerRight + const ui.Offset(80, 0),
    )!;
    expect(march.position.dx, expanded.right + 8);
    march.position += const ui.Offset(32, 0);
  ...
  testWidgets('五级原版建筑各不相同，降级和易主还原画面且不残留旧城堡', (tester) async {
    await tester.runAsync(() async {
      ...
      c.camera.resize(const ui.Size(240, 240));
      c.camera.center = city.bounds.bottomLeft + const ui.Offset(24, -32);
      ...
      expect(
        c.campaign.cityAt(city.bounds.bottomRight + const ui.Offset(8, -8)),
        isNull,
      );
      c.dispose();
      assets.dispose();
