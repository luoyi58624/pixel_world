...
(CampaignState, HeroMarch) _rescue({
  ...
  march.position = far ? const Offset(900, 800) : home + const Offset(15, -60);
...
void main() {
  ...
  test('R03 两城争用同一援军，只产生一次调动并检查原属城时限', () {
    ...
    invading.position =
        c.cityBounds(c.world.cities[3]).center + const Offset(130, 0);
    c.advance(1 / 60);
  ...
  test('R06/R07/R13 统一改令接口检查国家权限、战斗锁定与所属城存续', () {
    ...
    final march = c.dispatchTo(hero, const Offset(300, 500), countryId: 1)!;
    expect(c.moveTo(hero.id, const Offset(200, 600)), isFalse);
    expect(c.moveTo(hero.id, const Offset(200, 600), countryId: 2), isFalse);
    expect(c.moveTo(hero.id, const Offset(200, 600), countryId: 1), isTrue);
    ...
    expect(c.moveTo(hero.id, const Offset(100, 100), countryId: 1), isFalse);
