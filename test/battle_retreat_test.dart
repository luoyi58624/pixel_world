...
void main() {
  for (final (roll, success) in [
    ...
    test('撤退分位 $roll：六成失败边界准确，同场重复操作只判定一次', () {
      ...
      final gold = c.gold;
      final hp = hero.hp;
      expect(c.moveTo(hero.id, const Offset(220, 200)), isFalse);
  ...
  test('撤退沿实际折线路径反向移动，未到家不回血，回城归还生还兵员', () {
    ...
    const cornerA = Offset(260, 240), cornerB = Offset(600, 240);
    ...
    expect(c.moveTo(hero.id, const Offset(1000, 400)), isFalse);
  ...
  test('国家预算覆盖完整原路返程，不能只预留当前位置直线回城的粮草', () {
    final c = retreatCampaign();
    final hero = retreatHeroById(c, 0);
    const a = Offset(1000, 500), b = Offset(200, 500);
  ...
  test('野战任一方可撤退，同一对手未分离前不会立即再次交战', () {
    final c = retreatCampaign();
    final a = c.dispatchTo(retreatHeroById(c, 0), const Offset(500, 240))!;
    final b = c.dispatchTo(
      retreatHeroById(c, 3),
      const Offset(140, 240),
      countryId: 1,
    )!;
    a.position = const Offset(300, 240);
    b.position = const Offset(316, 240);
    c.advance(1 / 60);
    ...
    expect(a.destination, const Offset(500, 240));
    c.advance(1);
