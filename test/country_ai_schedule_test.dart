...
import 'package:pixel_world/world/ai/config.dart';
...
void main() {
  ...
  test('初始城防和守将占优时，不召回更强的在外将领', () {
    ...
    final march = c.dispatchTo(hero, const Offset(600, 100), countryId: 1)!;
    march.position =
        c.cityBounds(c.world.cities[1]).center + const Offset(15, -60);
  ...
  test('在途回援跨周期占用入城名额，复制与改派不会重复累计', () {
    ...
    final hero = c.garrisonAt(1).first;
    c.dispatchTo(hero, const Offset(600, 100), countryId: 1);
  ...
  test('已安排且能及时入城的强援军足够时，不再召回第二人', () {
    ...
    final a = c.dispatchTo(first, const Offset(600, 100), countryId: 1)!;
    final b = c.dispatchTo(second, const Offset(600, 100), countryId: 1)!;
    a.position = home + const Offset(0, -65);
    b.position = home + const Offset(20, -80);
    approaching(c, distance: 140);
    c.advance(1 / 60);
