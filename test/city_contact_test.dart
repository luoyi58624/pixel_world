...
final _pixels = <int, List<ui.Rect>>{};
...
void _expectTouch(CampaignState c, CityDefinition city, Offset center) {
  final origin = c.cityBounds(city).topLeft;
  final hero = ui.Rect.fromCenter(
...
void main() {
    ...
    for (final entry in cityAppearances.entries) {
      ...
      for (var y = 0; y < appearance.height * 16; y++) {
        for (var x = 0; x < appearance.width * 16; x++) {
          ...
          if (r == g && g == b) {
            pixels.add(ui.Rect.fromLTWH(x.toDouble(), y.toDouble(), 1, 1));
          }
        }
      }
  ...
  for (var level = 1; level <= 5; level++) {
    ...
    test('$level 级城八方向进攻直达墙体，首次接触才开始战斗且可撤离', () {
      for (final direction in _directions) {
        ...
        c.moveTo(hero.id, start);
        c.advance(0.3);
        expect(march.phase, MarchPhase.marching);
        expect(battle.isActive, isFalse);
        expect((march.position - destination).distance, greaterThan(0));
      }
    });
  }
  ...
  test('经过旗杆和图块顶部草地不会提前触发攻城，经过实际墙体仍会拦截', () {
    ...
    final bounds = c.cityBounds(city);
    final hero = c.garrisonAt(0).first;
    final start = bounds.topLeft + const Offset(-50, -4);
    final end = bounds.topRight + const Offset(50, -4);
    ...
    march.position = bounds.centerLeft - const Offset(50, 0);
    c.moveTo(hero.id, bounds.centerRight + const Offset(50, 0));
  ...
  test('点击城堡空白角仍抵达最近的有效墙体，途中不再二次修正停靠点', () {
    ...
    final hero = c.garrisonAt(0).first;
    final march = c.dispatch(hero, city)!;
    march.position = bounds.topRight + const Offset(80, -80);
    c.moveTo(hero.id, bounds.topRight + const Offset(-1, 1));
    final destination = march.destination;
  ...
  test('大步长和逐帧推进得到相同的贴城位置，不会穿过或提前卡住', () {
    ...
    for (final c in [a, b]) {
      ...
      march.position = c.cityBounds(city).center + const Offset(80, -80);
