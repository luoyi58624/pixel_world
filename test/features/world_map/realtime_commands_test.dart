...
void main() {
  ...
  test('任意空地出击、到达驻留、重新指定目标与扎营保留同一英雄身份', () {
    ...
    final point = c.heroPosition + const GamePoint(80, 32);
    c.openUnit(hero.id);
    ...
    c.openUnit(hero.id);
    c.prepareMove();
    c.confirmPosition(point - const GamePoint(0, 64));
  ...
  test('选点和取消不打断原行军，两个英雄的命令互不影响', () {
    ...
    final first = c.campaign.dispatchTo(
      heroes[0],
      c.heroPosition + const GamePoint(180, 16),
    )!;
    final second = c.campaign.dispatchTo(
      heroes[1],
      c.heroPosition + const GamePoint(120, 48),
    )!;
  ...
  test('移动可以选择我方城池，进驻后恢复驻军且不生成第二个英雄', () {
    ...
    final unit = c.campaign.dispatchTo(
      hero,
      c.heroPosition + const GamePoint(48, 16),
    )!;
    c.tick(3);
  ...
  test('自由行军线段进入敌城会拦下交战，撤离不会立刻被同一城重新吸回', () {
    ...
    final start = city.bounds.centerLeft - const GamePoint(10, 0);
    final end = city.bounds.centerRight + const GamePoint(48, 0);
    ...
    expect(
      c.campaign.moveTo(unit.hero.id, start - const GamePoint(32, 0)),
      isFalse,
    );
  ...
  test('选点期间英雄战败会清除过期命令，不会出现幽灵部队', () {
    ...
    c.tick(1);
    c.confirmPosition(c.heroPosition + const GamePoint(80, 0));
  ...
  testWidgets('角色点击、选点光标、情况、移动取消和扎营均在同一张地图完成', (tester) async {
    ...
    c.refreshUi();
    await tester.pump();
    await tester.tapAt(origin + (c.camera.toScreen(deployed.position)).toUi);
    await tester.pump();
    ...
    final point = c.heroPosition + const GamePoint(80, 32);
    await tester.tapAt(origin + (c.camera.toScreen(point)).toUi);
    await tester.pump(const Duration(milliseconds: 300));
    final unit = c.campaign.marches.values.single;
    expect(unit.destination, point);
    await tester.tapAt(origin + (c.camera.toScreen(unit.position)).toUi);
    await tester.pump();
    await tester.tap(find.byKey(const ValueKey('unit-move')));
    await tester.pump();
  ...
  testWidgets('地图与战场的鼠标拖拽都有短惯性，再次按住可立即截停', (tester) async {
    ...
    for (final watching in [false, true]) {
      ...
      camera.zoomTo(watching ? 6 : 3, camera.viewport.center(GamePoint.zero));
      camera.center = camera.worldSize.center(GamePoint.zero);
      c.refreshUi();
      await tester.pump();
  ...
  testWidgets('战场可以缩放拖动，后台部队继续行军，返回和重进不重置战斗', (tester) async {
    ...
    final other = c.campaign.dispatchTo(
      c.campaign.garrisonAt(0).first,
      c.heroPosition + const GamePoint(200, 48),
    )!;
    march.position = march.destination;
    c.tick(0.02);
    final battle = c.campaign.battles[1]!;
    ...
    c.battleCamera.zoomTo(4, c.battleCamera.viewport.center(GamePoint.zero));
    c.refreshUi();
    await tester.pump();
  ...
  testWidgets('小窗口进入观战，整场结束后自动回到地图并保留后台结果', (tester) async {
    ...
    await tester.tapAt(origin + (c.battleMarkerBounds(battle).center).toUi);
    await tester.pump();
