...
WorldCamera _camera([double scale = 3]) => WorldCamera(const GameSize(1024, 960))
  ..center = const GamePoint(512, 480)
  ..resize(const GameSize(800, 600))
  ..scale = scale;

void _fling(WorldCamera camera, GamePoint velocity) {
  camera.beginDrag();
...
void main() {
  test('松手延续拖动方向，高阻尼快速停住，额外位移不超过50屏幕像素', () {
    ...
    _fling(camera, const GamePoint(-10000, 0));
  ...
  test('惯性总位移不依赖帧率，缩放后保持相同屏幕滑行距离', () {
    ...
    for (final scale in [1.0, 3.0, 6.0]) {
      for (final frames in [1, 10, 60]) {
        ...
        final start = camera.center;
        _fling(camera, const GamePoint(600, -800));
  ...
  test('碰到边缘停止对应方向，不越界；全图时不会晃动', () {
    final camera = _camera();
    camera.center = GamePoint(camera.visibleWorld.width / 2 + 1, 480);
    _fling(camera, const GamePoint(1200, 0));
    camera.advanceInertia(0.02);
    expect(camera.visibleWorld.left, closeTo(0, 1e-8));
    expect(camera.coasting, isFalse);
    camera.overview();
    final overview = camera.center;
    _fling(camera, const GamePoint(-1000, -500));
  ...
  test('重新按住立即接管，取消后的迟到松手不重启惯性', () {
    final camera = _camera();
    _fling(camera, const GamePoint(-1000, 0));
    camera.advanceInertia(0.02);
    ...
    camera.cancelMotion();
    camera.endDrag(const GamePoint(-1000, 0));
    expect(camera.coasting, isFalse);
  });
  ...
  test('慢放、缩放手势、无效速度均不惯性，缩放或键盘接管时截停旧惯性', () {
    ...
    for (final velocity in [
      GamePoint.zero,
      const GamePoint(30, 20),
      const GamePoint(double.nan, 0),
    ]) {
      _fling(camera, velocity);
      expect(camera.coasting, isFalse);
    }
    camera.beginDrag();
    camera.endDrag(const GamePoint(1000, 0), allowInertia: false);
    expect(camera.coasting, isFalse);
    _fling(camera, const GamePoint(1000, 0));
    camera.zoomTo(4, const GamePoint(400, 300));
    expect(camera.coasting, isFalse);
    _fling(camera, const GamePoint(1000, 0));
    camera.pan(const GamePoint(10, 0));
    expect(camera.coasting, isFalse);
  });
}
