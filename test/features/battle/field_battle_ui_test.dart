...
void main() {
  for (final terrain in FieldTerrain.values) {
    testWidgets('${terrain.label}野战显示独立背景、双方减益，刀剑与角色入口共享战斗', (tester) async {
      ...
      final a = c.campaign.dispatchTo(hero, point + const GamePoint(40, 0))!;
      final b = c.campaign.dispatchTo(
        enemy,
        point - const GamePoint(40, 0),
        countryId: 1,
      )!;
      a.position = point - const GamePoint(8, 0);
      b.position = point + const GamePoint(8, 0);
      c.tick(0.02);
      ...
      await tester.tapAt(tester.getTopLeft(canvas) + (marker).toUi);
      await tester.pump();
