...
HeroMarch _leave(CampaignState c, int id, {int country = 0, double y = 40}) {
  ...
  final march = c.dispatchTo(hero, GamePoint(2000, y), countryId: country)!;
  march.position = GamePoint(200, y);
  return march;
}

void _return(CampaignState c, HeroMarch march, int id) {
  final city = c.world.cities[id];
  final point = c.cityBounds(city).centerLeft - const GamePoint(8, 0);
