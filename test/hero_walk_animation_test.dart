...
HeroMarch _march({int country = 0, Offset start = const Offset(160, 160)}) {
  ...
  final march = HeroMarch(
    hero: hero,
    position: start,
    destination: const Offset(1000, 160),
  );
...
void main() {
  ...
  test('跨越河岸和途中改道保持时钟连续，不随速度切换而重置', () {
    ...
    final a = _march(start: const Offset(180, 160));
    final b = _march(start: const Offset(180, 160));
    for (var i = 0; i < 13; i++) {
      a.tick(mixed, 0.1);
      b.tick(grass, 0.1);
  ...
  test('停止、到达、待战和交战不播放步态，再出发从第一帧开始', () {
    ...
    march.tick(world, 20);
    expect(march.animationStep, 0);
    march.moveTo(const Offset(1000, 160));
    march.tick(world, 0.1);
    expect(march.animationStep, 0);
    march.tick(world, 0.1);
    expect(march.animationStep, 1);
    march.moveTo(march.position + const Offset(0.1, 0));
    march.tick(world, 3);
